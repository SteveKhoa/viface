...
from benchmark.constant import N_REPEAT, TAR_IMAGES_FILE_PATH, BENCHMARK_RESULT_PATH, BENCHMARK_FUZZY_EXTRACTOR_MASK_THEN_LOCK_MASK_LENGTH
...
from benchmark.utils_data_transform_images_by_pair import DataTransformImagesByPair
from benchmark.utils_data_load_images_by_many_pair import DataLoadImagesByPairForMaskThenLock
...
class CaseMyFuzz(BenchmarkCase):
    def __init__(self, repeat: int = 500):
        ...
        data_extract = DataExtractTarToFileDictionary(TAR_IMAGES_FILE_PATH, load_fn=load_fn)
        file_dictionary = data_extract.extract()
        data_transform = DataTransformImagesByPair(repeat, file_dictionary)
        data_load = DataLoadImagesByPairForMaskThenLock()
