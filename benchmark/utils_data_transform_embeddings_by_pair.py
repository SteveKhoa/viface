...
class DataTransformEmbeddingsByPair:
    ...
    def _remove_folders_less_than_n_samples_then_get_names(self, file_dictionary: dict):
        ...
        print("DataTransformEmbeddingsByPair: number of people after filtered=", len(filtered_keys))
    ...
    @utils_time_measurement.decorator_time_measurement_log("DataTransformEmbeddingsByPair: transform the datasets")
    def transform(self):
        names = self._remove_folders_less_than_n_samples_then_get_names(
        ...
        for i in range(self.n_pairs):
            if i % 100 == 0:
                print(f"DataTransformEmbeddingsByPair: {i}\{self.n_pairs}")
